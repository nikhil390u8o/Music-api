...
╔══════════════════════════════════════════╗
   ...
   Built with FastAPI + yt-dlp + shrutibots
...
SHRUTIBOTS_URL = "https://shrutibots.site"
...
app = FastAPI(
    title="YouTube API",
    description="Search, Audio & Video download API",
    version="1.0.0",
    lifespan=lifespan,
)
...
# ─────────────────────────────────────────────
#  SHRUTIBOTS VIDEO DOWNLOAD HELPER
# ─────────────────────────────────────────────
async def _shrutibots_video(video_id: str, out: Path) -> bool:
    """
    shrutibots.site se video download karo.
    Step 1: token lo
    Step 2: stream URL se file save karo
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.get(
                f"{SHRUTIBOTS_URL}/download",
                params={"url": video_id, "type": "video"}
            )
            if r.status_code != 200:
                return False
            token = r.json().get("download_token")
            if not token:
                return False

        # Step 2 — file download karo
        stream_url = f"{SHRUTIBOTS_URL}/stream/{video_id}?type=video&token={token}"
        async with httpx.AsyncClient(
            timeout=httpx.Timeout(180, connect=10),
            follow_redirects=True
        ) as client2:
            async with client2.stream("GET", stream_url) as resp:
        ...
        print(f"[shrutibots_video error] {e}")
        if out.exists():
            try: out.unlink()
            except: pass
        return False
...
@app.get("/genkey")
async def genkey(
    label: str = Query(..., description="Is key ka naam"),
...
# ─────────────────────────────────────────────
#  4. VIDEO DOWNLOAD — shrutibots.site use karta hai
# ─────────────────────────────────────────────
@app.get("/video")
async def video(
    query:   str = Query(...),
    ...
    if not out.exists():
        # Step 1 — video_id nikalo
        opts = {**_base_opts(), "extract_flat": True}
        try:
            raw = await loop.run_in_executor(None, _run_ydl_no_dl, opts, url)
        except Exception as e:
            raise HTTPException(502, f"Video info failed: {e}")

        if not is_url and "entries" in raw:
            entries = raw.get("entries") or []
            if not entries:
                raise HTTPException(404, "Video nahi mila")
            raw = entries[0]

        video_id = raw.get("id", "")
        if not video_id:
            raise HTTPException(404, "Video ID nahi mila")

        # Step 2 — shrutibots se download karo
        success = await _shrutibots_video(video_id, out)
        if not success:
            raise HTTPException(502, "Video download failed — shrutibots API error")
...
async def stream_url(
    ...
    _check_key(x_api_key)
    if type == "video":
        return await video(query=query, quality=quality, background_tasks=background_tasks, x_api_key=x_api_key)
    else:
        return await audio(query=query, background_tasks=background_tasks, x_api_key=x_api_key)
